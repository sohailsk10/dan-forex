...
def max_check(currency, high, time_interval, currency_number, p_high, current_time, target_datetime):
    ...
    update_sql(sql)
    if max_high > p_high:
        _datetime_hit_high = str(datetime.utcnow())
        ...
        _hist_query = f"INSERT INTO historical_data (current_time, currency, time_interval, actual_high, actual_low, predicted_high, predicted_low, target_datetime, datetime_hit_high, datetime_hit_low) VALUES ({current_time}, {currency}, {time_interval}, {high}, {0}, {p_high}, {0}, {target_datetime}, {_datetime_hit_high}, {0})"
        # _hist_val = (current_time, currency, time_interval, high, 0, p_high, 0, target_datetime, _datetime_hit_high, 0)
        print(_hist_query)
        _hist_cursor = mydb.cursor()
        _hist_cursor.execute(_hist_query)
...
def min_check(currency, low, time_interval, currency_number, p_low, current_time, target_datetime):
    ...
    if min_low < p_low:
        ...
        # create another table for historical predictions
        _hist_query = f"INSERT INTO historical_data (current_time, currency, time_interval, actual_high, actual_low, predicted_high, predicted_low, target_datetime, datetime_hit_high, datetime_hit_low) VALUES ({current_time}, {currency}, {time_interval}, {0}, {low}, {0}, {p_low}, {target_datetime}, {0}, {_datetime_hit_low})"
        # _hist_val = (current_time, currency, time_interval, 0, low, 0, p_low, target_datetime, 0, _datetime_hit_low)
        print(_hist_query)
        _hist_cursor = mydb.cursor()
        _hist_cursor.execute(_hist_query)
